...
${nuy} ẉww.instagram.com/thenay_xploit_
╰──────────
╭─「 *BOT INFO* 」
${nuy} Sc Ori By: FxSx
           ...
           nuy.sendMessage(from, options, text, { quoted: mek } )
             ...
             case 'stag2':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					var buffer = fs.readFileSync(rano), contextInfo: { mentionedJid: mem, quoted: mek }
					nuy.sendMessage(from, buffer, sticker, { quoted: mek })
					fs.unlinkSync(rano)
					})
					break
              case 'stag':
                    ...
                    if ((isMedia && !lol.message.videoMessage || isQuotedSticker) && args.length == 0) {
                        ...
                        nuy.sendMessage(from, buffer, sticker, options)
