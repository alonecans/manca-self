...
✿➤ ᴋᴇᴛɪᴋ ${prefix}daftar`
				...
				const loli = fs.readFileSync('./mp3/selamatdatang.mp3')
     ...
     reply(mess.wait)
         ...
         if (isBanned) return reply(mess.only.benned)    
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					pinte = body.slice(11)
					anu = await fetchJson(`https://lindow-api.herokuapp.com/api/pinterest?search=${pinte}&apikey=LindowApi`, {method: 'get'})
					reply(mess.wait)
