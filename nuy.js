...
const fakeaudio = JSON.parse(fs.readFileSync('./database/json/fakeaudio.json'))
const fakedaftar = JSON.parse(fs.readFileSync('./database/json/fakedaftar.json'))
let {
...
✿➤ Ketik : ${prefix}daftar`
	...
	nuy.on('chat-update', async (mek) => {
        ...
        nuy.sendMessage(from, d, sticker, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "Selamat :V", 'jpegThumbnail': fs.readFileSync('./sticker/manca.webp')}}}})
        }
        ...
        if (fakeaudio.includes(messagesLink)){
        nuy.updatePresence(from, Presence.composing)
		const loli = fs.readFileSync('./mp3/notte.mp3')
        nuy.sendMessage(from, loli, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
        }
        if (fakedaftar.includes(messagesLink)){
        nuy.updatePresence(from, Presence.composing)
		const loli = fs.readFileSync('./mp3/terimakasih.mp3')
        nuy.sendMessage(from, loli, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
        }
		if (bad.includes(messagesLink)) {
		if (!isGroup) return
