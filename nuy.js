...
✿➤ Ketik : ${prefix}daftar`
	...
	nuy.on('chat-update', async (mek) => {
        ...
        nuy.sendMessage(from, d, sticker, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "Jangan Kasar² Dong", 'jpegThumbnail': fs.readFileSync('./sticker/manca.webp')}}}})
        }
        if (bad.includes(messagesLink)) {
        nuy.updatePresence(from, Presence.composing)
  ...
  break
			...
			if (!isUser) return reply(mess.only.userB)
                ...
                case 'nyimak':
					...
					const beb = nge[Math.floor(Math.random() * nge.length)]
					const an = pushname2[Math.floor(Math.random() * tod.length)]
					teks = `*Yang Paling Nyimak Di Grup Adalah :* @${beb.jid.split('@')[0]}`
					membr.push(beb.jid)
					mentions(teks, membr, true)
					break
				case 'brainly':
